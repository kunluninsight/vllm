...
VLLM_SIMULATOR_OUTPUT_DIR by the profile hook on each round boundary
...
from vllm_simulator.simulation.benchmark import BaseWorker
from vllm_simulator.utils import get_logger
from vllm_simulator.simulation.vllm.startup import init_hook
...
class VLLMWorker(BaseWorker):
    ...
    def __init__(
        ...
        # dumps request/iteration stats here (read back via Envs.output_dir(),
        # which resolves the same VLLM_SIMULATOR_OUTPUT_DIR).  setdefault so
        # an explicit VLLM_SIMULATOR_OUTPUT_DIR from the environment wins.
        os.environ.setdefault(
            "VLLM_SIMULATOR_OUTPUT_DIR", f"/tmp/vllm_simulator/{name}"
        )
        self.output_dir = os.path.realpath(os.environ["VLLM_SIMULATOR_OUTPUT_DIR"])
    ...
    async def trigger_simulation(self, output_dir: str | None = None):
        ...
        request/iteration stats to VLLM_SIMULATOR_OUTPUT_DIR and resets
