"""Test vLLM framework hijacking via vllm_simulator hooks.
...
from vllm_simulator.dataset import DatasetArgs, SimpleDataset, get_dataset
from vllm_simulator.simulation.benchmark import BenchmarkConfig, MultiInstanceBenchmarkRunner
from vllm_simulator.simulation.vllm.vllm_worker import VLLMWorker, EngineArgs
...
def test_vllm_benchmark():
    model_engine_args_list = [
        {"model": "/nfs/lvm/models/Qwen/Qwen3-8B/"},       # MHA
        {
            "model": "/nfs/lvm/models/Qwen/Qwen3.5-9B/",     # Hybrid (full_attention + linear_attention)
            # Native mamba state pages are ~2MB/layer (vs the old uniform-64KB
            # padding), so each KV block costs ~68MB. Size the host pool so
            # the eviction round (~204 blocks) cannot evict the cached prompts
            # (~32 blocks): 32 GiB / 68MB ≈ 477 blocks.
            "kv_offloading_size": 32.0,
        },
    ]
